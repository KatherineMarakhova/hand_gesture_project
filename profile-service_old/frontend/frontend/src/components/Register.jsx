    ...
    <div className="login-form">
      ...
      <form onSubmit={handleRegister}>
        <div className="form-group">
          ...
          {(usernameError) ? (<div style={{color:'#D33F49'}}>{usernameError}</div>) : (<div></div>)}
          <input onChange={(e) => usernameHandler(e)} name="username" type="text" className="form-control" value={username} required />
        </div>

        <div className="form-group">
          <label className="form-label">Email</label>
          {(emailError) ? (<div style={{color:'#D33F49'}}>{emailError}</div>) : (<div></div>)}
          <input onChange={e => emailHandler(e)} name="email" type="email" className="form-control" value={email} required />
        </div>

        <div className="form-group">
          <label className="form-label">Пароль</label>
          {<div style={{color:'#D33F49'}}>{passwordError}</div>}
          <input onChange={e => passwordHandler(e)} name="password" type="password" className="form-control" value={password} required />
        </div>

        <div className="form-group">
          <label className="form-label">Повторите пароль</label>
            {(password.length === dup_password.length && password !== dup_password)?(<div style={{color:'#D33F49'}}>Пароли не совпадают</div>) : (<div></div>)}
          <input onChange={e => {setDupPassword(e.target.value)}} name="dup_password" type="password" className="form-control" value={dup_password} required />
        </div>
